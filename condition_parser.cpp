...
shared_ptr<Node> ParseExpression(It& current, It end, unsigned precedence) {
  ...
  while (current != end && current->type != TokenType::PAREN_RIGHT) {
    ...
    const auto logical_operation = current->value == "AND" ? LogicalOperation::And
                                                           : LogicalOperation::Or;
    const auto current_precedence = precedences.at(logical_operation); //unsigned int
...
void TestParseCondition() { 
    {
      istringstream is("");
      AssertEqual(ParseCondition(is)->NodeType, "EmptyNode", "Parse Condition with empty entry");
    }
    
    {
      istringstream is("date > 2017-11-20");
      AssertEqual(ParseCondition(is)->NodeType, "DateComparisonNode", "Parse Condition with date");
    }

    {
      istringstream is("event != \"working day\"");
      AssertEqual(ParseCondition(is)->NodeType, "EventComparisonNode", "Parse Condition with event");
    }

    {
      istringstream is("date >= 2017-01-01 AND date < 2017-07-01");
      AssertEqual(ParseCondition(is)->NodeType, "LogicalOperationNode", "Parse Condition with logical operators");
    }
}